...
void sample_curve_to_track (Curve *curve, Track *track, double theta)
{
  ...
  sample_bezier_curve (bez_points, SAMPLE_THETA, 
...
void shoot_ammo (Game *game, double sx, double sy)
{
	...
	if ( (count < SHOT_MAX) && (game->canon.reload < game->canon.reload_time) )
	{
				...
				if (rz  != 0)
				{
					...
					if (p > 0) // On insere la marble avant
					{
						if (k == marble_count - 1)
						{
							...
							game->track_list.tracks[j].marbles[k].t, game->track_list.tracks[j].sample_count, dist, &xB, &yB);
						...
						else
						{
							...
							{
								result = compute_distant_point_forward (game->track_list.tracks[j].sample_x, game->track_list.tracks[j].sample_y, 
									game->track_list.tracks[j].marbles[l].t, game->track_list.tracks[j].sample_count, dist, &xB, &yB);
							...
							memmove (game->track_list.tracks[j].marbles + k + 2,game->track_list.tracks[j].marbles + k + 1, sizeof (Marble)*(marble_count -k));
							game->track_list.tracks[i].marble_count++;

							firstMarble.color = game->shot_list.shots[i].color;
							game->track_list.tracks[i].marbles[k + 1] = firstMarble;
					...
					else // On insere apres
					{
						...
						memmove (game->track_list.tracks[j].marbles + k + 1,game->track_list.tracks[j].marbles + k, sizeof (Marble)*(marble_count -k));
						game->track_list.tracks[i].marble_count++;
						firstMarble.color = game->shot_list.shots[i].color;
